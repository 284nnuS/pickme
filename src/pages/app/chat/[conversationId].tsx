...
import { ChatBox, ConversationItem } from '~/src/components'
...
function Index({
   ...
   useEffect(() => {
      socket.open()
      socket
         .on('Remove match', (otherId: number) => {
            ...
            <div className="w-[30rem] min-w-[30rem] h-screen z-50">
               ...
               <div className="w-full h-[90%] bg-slate-100">
                  ...
                  {result && result.length > 0 ? (
                     ...
                     result && (
                        <div className="mt-6 text-xl text-center text-bold text-slate-600">No conversations found</div>
                     )
                  )}
               </div>
            </div>
               ...
               {currentConversation.conversationId && (
                  <ChatBox
                     ...
                     unMatchCallback={() => {
                        ...
                        //    const idx = current.findIndex((e) => e.userId === other.userId)
                        //    current.splice(idx, 1)
                        //    return [...current]
                        // })
                        // setDeleted(true)
